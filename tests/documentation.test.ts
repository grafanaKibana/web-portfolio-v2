...
test("Code activity keeps the compact pull-request row structure", () => {
  ...
  const meta = slots.get("pull-request-meta");
  const period = slots.get("pull-request-date");
  const diff = slots.get("pull-request-diff");
  assert.ok(row && status && copy && title && meta && period && diff);
  ...
  assert.match(meta.getText(sourceFile), /styles\.meta/);
  assert.match(period.getText(sourceFile), /styles\.period/);
  assert.match(diff.getText(sourceFile), /styles\.diff/);
  assert.ok(status.getStart() < copy.getStart() && copy.getStart() < meta.getStart());
  assert.ok(meta.getStart() < period.getStart() && period.getStart() < diff.getStart());
  ...
  const diffSource = diff.parent.getText(sourceFile);
  assert.equal(diffSource.match(/aria-hidden="true"/g)?.length, 2);
  assert.equal(diffSource.match(/className="sr-only"/g)?.length, 1);
  assert.match(diffSource, /\+\{pullRequestCount\.format\(contribution\.additions\)\}/);
  assert.match(diffSource, /−\{pullRequestCount\.format\(contribution\.deletions\)\}/);
  assert.match(diffSource, /contribution\.additions === 1 \? "addition" : "additions"/);
  assert.match(diffSource, /contribution\.deletions === 1 \? "deletion" : "deletions"/);
  assert.equal(source.match(/new Intl\.NumberFormat\("en-US"\)/g)?.length, 1);
  assert.doesNotMatch(source, /^["']use client["'];?/m);
  assert.doesNotMatch(source, /pull-request-summary|contribution\.summary|body_text/);
  assert.doesNotMatch(source, /changedFiles|changed-files|tooltip/i);
});
