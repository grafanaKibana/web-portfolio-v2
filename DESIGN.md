...
Give the page generous gutters and consistent pauses between sections, with smaller gaps inside related groups. Home uses Tailwind utilities for ordinary layout, spacing, and typography. Main repeated entries share one compact spacing step and one larger desktop step; divided editorial rows split that interval across their two sides. Sections and their labels follow the same spacing utilities throughout Home. Skills groups have no separate group gap; equal vertical margins around each skills list separate it from the surrounding headings. The activity calendar appears only inside Show more, after the final contribution list, with vertical spacing around it. Compact credential and activity rows retain their denser internal spacing. Structural layout changes keep their own content-driven breakpoints. Keep metadata secondary and align long-form reading directly to the shared editorial shell. Headings separate reading sections more strongly than paragraphs; the first content block sits close to its header divider. Thin dividers and open rows provide structure. Reserve rounded surfaces and shadows for controls and overlays that need separation.
