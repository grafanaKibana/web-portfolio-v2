...
The hero introduces Nikita and offers direct contact, résumé, and experience actions. Its server-rendered text, descriptor, and actions retain their staggered fade-and-rise reveal. The reveal starts from the first styled frame without waiting for background assets or hydration; on the first visit, it follows the opening splash handoff. The header and AI entry line join the opening reveal without waiting for hydration. Reduced motion uses a short fade without translation; rotating descriptors overlap their transitions and use a brief opacity-only change under that preference. About uses first-person overview copy and reveals the biography and individual career chapters independently. Experience carries the detailed timeline and recommendations, with previous/next controls when recommendations overflow. Projects and writing provide deeper evidence before Contact; repeated editorial titles use equal emphasis, internal destinations use straight arrows, and Writing shows its existing publication dates.
...
Use Geist Sans for headings and reading, and Geist Mono for dates, labels, counts, and code. Keep overview prose compact and long-form reading more spacious, with metadata and footer links sharing a quiet secondary scale. Let the hero scale down naturally on tablet and phone. Home section headings use sentence-case Geist Sans at 24px/600 without top rules. Structural group headings use 18px/600 and retain their separator rules, including the centered Skills rules. Record titles use 16px/500 beneath their group headings; independent project and article titles use 20px/500. About and Experience keep their distinct role headings. These values establish the approved plain-reference hierarchy while section-heading treatments remain under exploration.

Give the page generous gutters and consistent pauses between sections, with smaller gaps inside related groups. Home uses Tailwind utilities for ordinary layout, spacing, and typography. Main repeated entries share one compact spacing step and one larger desktop step; divided editorial rows split that interval across their two sides. Sections and their labels follow the same spacing utilities throughout Home. Skills groups have no separate group gap; equal vertical margins around each skills list separate it from the surrounding headings. The activity calendar appears inside More activity, after the final contribution list, with a Contributions (X) caption and trailing intensity legend. Its date range remains available to assistive technology. Disclosure copy stays short; its accessible name explains the additional content. Unavailable activity has an explicit recovery link, and a calendar-only response is named as such. Compact credential and activity rows retain their denser internal spacing. Structural layout changes keep their own content-driven breakpoints. Keep metadata secondary and align long-form reading directly to the shared editorial shell. Headings separate reading sections more strongly than paragraphs; the first content block sits close to its heading. Thin dividers and open rows provide structure. Reserve rounded surfaces and shadows for controls and overlays that need separation.

### Icons and imagery

Use Lucide for interface actions and recognizable technology marks for skills and external identities. Action-link and disclosure icons share the hero buttons' 1rem size, including credential verification cues; icon sizing does not reduce their touch targets. Preserve the existing solid, gradient, and multicolor skill treatments where they convey identity or category. Skills regain their brand color on hover and while their name tooltip is open, across compact and desktop layouts; skill labels use heading color on hover, keyboard focus, and while the tooltip is open. Avoid replacing all marks with one universal brand color. Keep assets local and decorative icons hidden from assistive technology.
...
| Footer | Desktop has navigation with space-between, then copyright and local time in a second space-between row. Tablet and mobile retain two link columns, followed by left-aligned copyright and local time on separate rows. Enlarged labels may use one link column. |
...
Contact places its introduction and direct links in one wide-screen row, with the links using only their intrinsic width on the right. Links use two compact columns in source order, with E-Mail as the visible email label and the address retained in its accessible name. Narrow or enlarged layouts stack the two groups and may reduce the link grid to one column. Align compact contact links to the reading edge. Let other social links and skill groups wrap naturally, with comfortable touch targets for compact skill icons and project actions. On phones, contribution dates and change counts sit below the title so long pull-request names retain a full reading line. Keep reading order intact and use split layouts only while both groups remain comfortable. Compact navigation and conversation fit the available viewport, including the on-screen keyboard. Touch actions remain available without hover.
