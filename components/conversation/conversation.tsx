      ...
      {capability !== "unsupported" ? (
        <>
          ...
          >
            ...
            {capability === "supported" && !open && !model.expanded ? <div className={styles.entryField} id="portfolio-conversation-entry" inert={!entryRevealed}><ConversationComposer entry mobile={mobile} model={model} onAccepted={handleAccepted} /></div> : null}
            {capability === "supported" ? <Button aria-controls={open || model.expanded ? "portfolio-conversation" : "portfolio-conversation-entry"} aria-expanded={model.expanded ? open : entryRevealed} aria-haspopup={model.expanded ? "dialog" : undefined} aria-label={model.expanded ? "Reopen conversation" : "Ask about my work"} className={styles.edgeButton} data-launcher onClick={handleLauncherClick} onPointerDown={(event) => { if (!model.expanded && !open) { event.preventDefault(); handleLauncherClick(); } }} ref={launcherRef} tabIndex={entryRevealed && !model.expanded ? -1 : undefined} type="button" variant="ghost">
